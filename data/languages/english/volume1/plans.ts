...
function createPagePlanItems(totalDays: number): ReadingPlanItem[] {
  const pagesPerDay = Math.ceil(ENGLISH_VOLUME1_TOTAL_PAGES / totalDays);

  return Array.from({ length: totalDays }, (_, index) => {
    const day = index + 1;
    const startPage = index * pagesPerDay + 1;
    const endPage = Math.min(day * pagesPerDay, ENGLISH_VOLUME1_TOTAL_PAGES);
...
const guidedSectionItems: ReadingPlanItem[] = ENGLISH_VOLUME1_SECTIONS.map(
  (section, index) => ({
    day: index + 1,
    label: section.title,
...
export const ENGLISH_VOLUME1_PLANS: ReadingPlan[] = [
  {
    id: "guided-sections",
    title: "Guided Section Journey",
    description:
      "Read the English edition by its real topics: front matter, parts, and chapters.",
    totalDays: guidedSectionItems.length,
    pagesPerDay: Math.ceil(ENGLISH_VOLUME1_TOTAL_PAGES / guidedSectionItems.length),
  ...
  },
  {
    ...
    totalDays: 30,
    pagesPerDay: Math.ceil(ENGLISH_VOLUME1_TOTAL_PAGES / 30),
  ...
  },
  {
    ...
    description:
      "Smaller daily portions for a calmer pace through the English edition.",
    totalDays: 49,
    pagesPerDay: Math.ceil(ENGLISH_VOLUME1_TOTAL_PAGES / 49),
